// TOKEN MAPBOX ANDA
mapboxgl.accessToken = 'pk.eyJ1IjoiaWJudXplZSIsImEiOiJjbWloMzBhcmMwNDFtM2ZyMW82OXI1bGh6In0.Vmad2LT_HRQFZLxtA9jDWQ';

let map;
let busMarkers = {};
let historyLineId = null; // ID Layer History
let followMode = false;
let showHistory = false;
let cameraMode = '2D'; // Default 2D
let lastAlert = 0;
...
// Hitung Batas Area agar pas di tengah
...
function initMap() {
    ...
    const styleUrl = savedTheme === 'dark' ? 'mapbox://styles/mapbox/dark-v11' : 'mapbox://styles/mapbox/light-v11';
    
    // Set atribut HTML untuk CSS
...
function addStops() {
    stops.forEach(stop => {
        ...
        new mapboxgl.Marker(el)
            ...
            .setPopup(new mapboxgl.Popup({ offset: 25 }).setText(stop.title))
            .addTo(map);
    });
}
...
function add3DBuildings() {
    ...
    if (!map.getLayer('add-3d-buildings')) {
        map.addLayer({
            'id': 'add-3d-buildings', 'source': 'composite', 'source-layer': 'building', 'filter': ['==', 'extrude', 'true'], 'type': 'fill-extrusion', 'minzoom': 14,
...
function setupControls() {
    // 1. LIVE CAM (3D/2D)
    const liveBtn = document.getElementById('liveCamBtn');
    const liveLabel = liveBtn.querySelector('.btn-label');
    ...
    liveBtn.onclick = function() {
        ...
        if (followMode) {
            ...
            cameraMode = '3D';
            liveLabel.innerText = '3D';
            const firstBus = Object.keys(busMarkers)[0];
            if (firstBus) map.flyTo({ center: busMarkers[firstBus].getLngLat(), zoom: 17.5, pitch: 60, bearing: -20, speed: 1.2 });
        } else {
            this.classList.remove('active');
            cameraMode = '2D';
            liveLabel.innerText = '2D';
    ...
    // 3. HISTORY JEJAK (FITUR YANG SEMPAT HILANG)
    const historyBtn = document.getElementById('historyBtn');
    historyBtn.onclick = function() {
        showHistory = !showHistory;
        this.classList.toggle('active'); // Warna tombol jadi merah

        if (showHistory) {
            fetchHistory(); // Ambil data dan gambar garis
        } else {
            // Hapus garis history
    ...
    // 4. THEME TOGGLE (FITUR YANG SEMPAT HILANG)
    // Catatan: Di Mapbox, ganti tema = reload style map, jadi layer harus digambar ulang
    const themeBtn = document.getElementById('themeToggleBtn'); // Pastikan ID ini ada di HTML floating-controls (tambahkan jika belum)
    if(themeBtn) {
        themeBtn.onclick = () => {
            const current = document.documentElement.getAttribute('data-theme');
            const next = current === 'dark' ? 'light' : 'dark';
            const styleUrl = next === 'dark' ? 'mapbox://styles/mapbox/dark-v11' : 'mapbox://styles/mapbox/light-v11';
            ...
            // Tunggu style baru load, lalu gambar ulang rute & gedung
            map.once('style.load', () => {
                add3DBuildings();
                addRoutes();
                // Marker halte & bus otomatis tetap ada (karena elemen DOM terpisah)
...
function updateMarker(bus) {
    ...
    if (followMode) {
        const pitch = cameraMode === '3D' ? 60 : 0;
        const bearing = cameraMode === '3D' ? -20 : 0;
        map.easeTo({ center: pos, pitch: pitch, bearing: bearing, duration: 1000 });
    }

    const content = `<div class="iw-header"><img src="/static/images/bipol.png" class="iw-icon"><h3>${bus.bus_id}</h3></div><p>Speed: ${bus.speed} km/h<br>Gas: ${bus.gas_level}</p>`;
    ...
    if (busMarkers[bus.bus_id]) {
        ...
        const el = document.createElement('div');
        el.style.backgroundImage = 'url(/static/images/bipol.png)';
        el.style.width = '40px'; el.style.height = '40px'; el.style.backgroundSize = 'cover';
        busMarkers[bus.bus_id] = new mapboxgl.Marker(el).setLngLat(pos).setPopup(new mapboxgl.Popup({ offset: 25 }).setHTML(content)).addTo(map);
    }
}
...
function updateSidebar(bus, list) {
    ...
    item.innerHTML = `<div class="bus-icon-wrapper"><img src="/static/images/bipol.png"></div><div class="bus-info"><h4>${bus.bus_id} <span class="status-dot ${statusDot}"></span></h4><p><span><i class="fa-solid fa-gauge"></i> ${bus.speed}</span> <span><i class="fa-solid fa-fire"></i> ${bus.gas_level}</span></p></div>`;
    
    item.onclick = () => {
        map.flyTo({ center: [bus.longitude, bus.latitude], zoom: 18 });
        followMode = true;
        document.getElementById('liveCamBtn').click(); 
    };
    list.appendChild(item);
}

// FUNGSI HISTORY (JEJAK)
async function fetchHistory() {
    try {
        const res = await fetch('/api/bus/history'); // Panggil API backend yang sudah kita buat tadi
        const json = await res.json();
        const points = json.data.map(p => [p.lng, p.lat]); // Mapbox [Lng, Lat]

        // Hapus layer lama jika ada
        if (map.getLayer('historyLayer')) map.removeLayer('historyLayer');
        if (map.getSource('historySource')) map.removeSource('historySource');

        // Gambar Garis Baru (Oranye)
        ...
        map.addLayer({
            ...
            paint: { 'line-color': '#e67e22', 'line-width': 4, 'line-dasharray': [2, 2] } // Garis putus-putus
        });
...
function calculateETA(bus) {
    ...
    if (minDist < 0.05) { etaEl.innerHTML = `<i class="fa-solid fa-check-circle"></i> Tiba di ${nearestStop}`; etaEl.classList.remove('hidden'); }
    else { const time = Math.ceil((minDist / Math.max(bus.speed, 20)) * 60); etaEl.innerHTML = `<i class="fa-solid fa-clock"></i> ${time} min ke ${nearestStop}`; etaEl.classList.remove('hidden'); }
}
